...
def run_experiment_stopgo(cf_model_class, ode_integrator_class, h=1.0, t_max=60.0, max_vehicle_count=20,
    ...
    if experiment_name is None:
        experiment_name = 'stopgo_' + get_experiment_name(cf_model_class, ode_integrator_class, h)
    # Create file handler which logs even debug messages
    fh = logging.FileHandler(experiment_name + '.log', mode='w')
    fh.setLevel(logging.DEBUG)
    formatter = logging.Formatter(
...
def get_experiment_data(experiment_name, vehicle_id, resample_period=0, min_time=0.0, max_time=0.0):
    try:
        ...
        for i in range(len(ref_t)):
            ...
            if do_output_this_row:
                ...
                panm_globals.LOGGER.info('[{:d}] t={:4.1f}, spd={:5.2f}, dst={:5.2f}'.format(row, t, ref_spd[i], ref_dst[i]))
                row += 1
...
def get_global_error(exp_data, ref_data):
...
if False:
    ...
    for integrator in integrators:
        ...
        for th in times:
            ...
            global_error = get_global_error(exp_data, ref_data)
...
if False:
    # For selected vehicle id, generate vehicle and position plots for particular integration scheme and all
